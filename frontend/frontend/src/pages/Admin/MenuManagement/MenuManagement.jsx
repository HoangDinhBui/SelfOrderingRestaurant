...
const MenuManagement = () => {
  ...
  const [filteredDishes, setFilteredDishes] = useState(dishes); // Lưu danh sách món ăn được lọc
  ...
  const confirmDeleteDish = () => {
    const updatedDishes = dishes.filter((dish) => dish.id !== dishToDelete.id); // Xóa món ăn
    setDishes(updatedDishes); // Cập nhật danh sách món ăn
    setFilteredDishes(updatedDishes); // Cập nhật danh sách hiển thị
  ...
  const validateAndAddDish = () => {
    ...
    const updatedDishes = [
      ...dishes,
      { ...newDish, id: `A${dishes.length + 1}` }, // Tạo ID tự động
    ];
    setDishes(updatedDishes); // Cập nhật danh sách món ăn
    setFilteredDishes(updatedDishes); // Cập nhật danh sách hiển thị
  ...
  const handleSearch = (event) => {
    if (event.key === "Enter") {
      if (searchTerm.trim() === "") {
        setFilteredDishes(dishes); // Hiển thị toàn bộ danh sách nếu ô tìm kiếm trống
      } else {
